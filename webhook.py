...
def notify_section_availability(section: dict):
    ...
    payload = {
        ...
        "embeds": [{
            ...
            "Section open!",
            "timestamp":
            f"{datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')}",
            "color":
            2150418,
            "fields": [{
                "name": "Course",
                ...
                "**Register**",
                "value":
                "[WebReg](https://sims.rutgers.edu/webreg/editSchedule.htm?login=cas&"
...
def check_active_courses():
    ...
    for request in snipes.find():
        if request['index'] not in open_sections and request[
                'status'] == 'open':
            logger.info(f"Index {request['index']} has been closed :(")
            snipes.update_one({"index": request['index']},
                              {"$set": {
                                  "status": "closed"
                              }})
            continue
        elif request['index'] not in open_sections and request[
                'status'] == 'closed':
            logger.info(f"Index {request['index']} is currently closed.")
            continue
        elif request['index'] in open_sections and request['status'] == 'open':
            logger.info(
                f"Notified users already that index {request['index']} is open."
            )
            continue
        else:
            snipes.update_one({"index": request['index']},
        ...
        pthreads.append(
            multiprocessing.Process(target=notify_section_availability,
                                    args=(request, )))
        pthreads[x].start()
        x += 1
